...
def test_plan_command_returns_json_and_zero_writes_when_dry_run() -> None:
    ...
    result = runner.invoke(app, ["plan", "--source", "uf", "--year", "2024", "--dry-run"])
...
def test_source_inspect_and_local_execute_expose_real_subapp_surfaces(tmp_path: Path) -> None:
    ...
    inspected = runner.invoke(app, ["source", "inspect", "--source", "uf"])
    executed = runner.invoke(app, ["run", "--source", "uf", "--year", "2024", "--execute"])
