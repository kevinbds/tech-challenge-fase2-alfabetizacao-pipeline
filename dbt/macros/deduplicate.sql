{% macro deduplicate(relation, key_columns, payload_columns) %}
    {% if adapter is defined %}
        {{ return(
            adapter.dispatch('deduplicate', 'alfabetizacao_medallion')(
                relation,
                key_columns,
                payload_columns
            )
    ...
    {% endif %}
    {{ bigquery__deduplicate(relation, key_columns, payload_columns) }}
{% endmacro %}

{% macro bigquery__deduplicate(relation, key_columns, payload_columns) %}
, hashed as (
  select
    *,
...
{% macro duckdb__deduplicate(relation, key_columns, payload_columns) %}
, hashed as (
  select
    *,
    sha256(to_json(struct_pack(
      {% for column in payload_columns %}{{ column }} := {{ column }}{% if not loop.last %}, {% endif %}{% endfor %}
    ))) as _row_hash
  from {{ relation }}
), classified as (
  select
...
select * exclude (_row_hash, _duplicate_count, _distinct_hashes)
from classified
where _distinct_hashes = 1
qualify row_number() over (
  partition by {% for column in key_columns %}{{ column }}{% if not loop.last %}, {% endif %}{% endfor %}
